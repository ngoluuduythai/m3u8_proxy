...
package uniffi.M3U8ProxyServer
...
internal interface _UniFFILib : Library {
    companion object {
        internal val INSTANCE: _UniFFILib by lazy {
            loadIndirect<_UniFFILib>(componentName = "M3U8ProxyServer")
                .also { lib: _UniFFILib ->
                    FfiConverterTypeZenDataChannel.register(lib)
                }
        }
    }
...
public object FfiConverterTypeM3u8ProxyServer : FfiConverter<M3u8ProxyServer, Pointer> {
    override fun lower(value: M3u8ProxyServer): Pointer = value.callWithPointer { it }

    override fun lift(value: Pointer): M3u8ProxyServer {
        return M3u8ProxyServer(value)
    }

    override fun read(buf: ByteBuffer): M3u8ProxyServer {
        // The Rust code always writes pointers as 8 bytes, and will
        // fail to compile if they don't fit.
        return lift(Pointer(buf.getLong()))
    }

    override fun allocationSize(value: M3u8ProxyServer) = 8

    override fun write(value: M3u8ProxyServer, buf: ByteBuffer) {
...
// Declaration and FfiConverters for ZenDataChannel Callback Interface

public interface ZenDataChannel {
    fun `getTsSegment`(`name`: String): List<UByte>?
    fun `getMasterPlaylist`(): List<UByte>?
}

// The ForeignCallback that is passed to Rust.
internal class ForeignCallbackTypeZenDataChannel : ForeignCallback {
    @Suppress("TooGenericExceptionCaught")
    override fun invoke(handle: Handle, method: Int, args: RustBuffer.ByValue, outBuf: RustBufferByReference): Int {
        val cb = FfiConverterTypeZenDataChannel.lift(handle)
        return when (method) {
            IDX_CALLBACK_FREE -> {
                FfiConverterTypeZenDataChannel.drop(handle)
                // No return value.
                // See docs of ForeignCallback in `uniffi/src/ffi/foreigncallbacks.rs`
                0
            }
            1 -> {
                val buffer = this.`invokeGetTsSegment`(cb, args)
            ...
            2 -> {
                val buffer = this.`invokeGetMasterPlaylist`(cb, args)
                outBuf.setValue(buffer)
                // Value written to out buffer.
                // See docs of ForeignCallback in `uniffi/src/ffi/foreigncallbacks.rs`
                1
    ...
    private fun `invokeGetTsSegment`(kotlinCallbackInterface: ZenDataChannel, args: RustBuffer.ByValue): RustBuffer.ByValue =
        try {
            val buf = args.asByteBuffer() ?: throw InternalException("No ByteBuffer in RustBuffer; this is a Uniffi bug")
            kotlinCallbackInterface.`getTsSegment`(
    ...
    private fun `invokeGetMasterPlaylist`(kotlinCallbackInterface: ZenDataChannel, args: RustBuffer.ByValue): RustBuffer.ByValue =
        try {
            kotlinCallbackInterface.`getMasterPlaylist`()
                .let {
                    FfiConverterOptionalSequenceUByte.lowerIntoRustBuffer(it)
                } // TODO catch errors and report them back to Rust.
            // https://github.com/mozilla/uniffi-rs/issues/351
...
public object FfiConverterTypeZenDataChannel : FfiConverterCallbackInterface<ZenDataChannel>(
    foreignCallback = ForeignCallbackTypeZenDataChannel()
) {
